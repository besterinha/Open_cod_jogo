...
- **Self-test de pacote:** `bash ci/export-selftest.sh` exporta binário Linux headless e roda `--selftest` DENTRO dele (cena `content/selftest/selftest.tscn` + autoload boot) verificando terreno/tiles/HUD/conteúdo de abilities — exit 0/1 bloqueia CI; é o que reproduz bugs "só no device" sem device. Com `xvfb-run` disponível roda em modo VISUAL: imprime dump JSON de estado (`[SelfTest] state={terreno,units}`) + salva screenshot 720p em `user://selftest.png` (copiada p/ `build/selftest/screenshot.png`) — fecha o buraco "verificação visual sem device"; libs X11 mínimas: libxcursor1 libxinerama1 libxrandr2 libxi6 + mesa
