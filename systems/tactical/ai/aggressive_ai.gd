...
func generate_intent(unit: Unit, board: TacticalBoard) -> Dictionary:
	...
	enemies.sort_custom(
		func(a: Unit, b: Unit) -> bool: return a.get_stat(focus_stat_id) < b.get_stat(focus_stat_id)
	)
	...
	var reachable: Array[Vector2i] = board.grid.get_reachable(
		unit.cell,
		unit.get_stat("movement"),
		func(c: Vector2i) -> bool: return board.is_walkable(c) or c == unit.cell
	)
	...
	for c in reachable:
		...
		if d < best_dist:
			best_dist = d
			best = c
	return {"move_to": best, "target": target.cell, "target_unit": target}
