...
func test_reachable_range_1() -> void:
	var g := GridSystem.new(Vector2i(8, 8), 1.0)
	var reachable: Array[Vector2i] = g.get_reachable(Vector2i(4, 4), 1, func(_c: Vector2i) -> bool: return true)
	# origem + 4 vizinhos = 5
	assert_eq(reachable.size(), 5)

func test_reachable_blocked() -> void:
	var g := GridSystem.new(Vector2i(3, 3), 1.0)
	var reachable: Array[Vector2i] = g.get_reachable(Vector2i(1, 1), 2, func(c: Vector2i) -> bool: return c != Vector2i(2, 1))
