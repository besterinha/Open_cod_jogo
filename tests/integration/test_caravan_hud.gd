...
func test_caravan_bar_mostra_supplies_e_morale() -> void:
	...
	await get_tree().process_frame
	assert_true(
		label.text.contains("25") or label.text.contains("70") or label.text.contains("3"),
		"CaravanBar deve mostrar dados do caravan (25/70/3), got: %s" % label.text
	)
...
func test_radial_menu_acoes_chamam_sistemas() -> void:
	...
	if arena.has_method("_on_radial_action"):
		...
		assert_true(
			after >= before_supplies - 10 and after <= before_supplies + 15,
			(
				"Supplies após viajar deve estar em [%d, %d], got %d (before %d)"
				% [before_supplies - 10, before_supplies + 15, after, before_supplies]
			)
		)
